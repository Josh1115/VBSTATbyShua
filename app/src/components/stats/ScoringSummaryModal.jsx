...
function BookkeeperView({ rallies, contacts }) {
  ...
  const jerseyMap = useMemo(() => {
    ...
    return Object.fromEntries(teamPlayers.map((p) => [p.id, p.jersey_number ?? '?']));
  }, [teamPlayers]);

  const { usPoints, themPoints, rallyScores } = useMemo(() => {
    if (!rallies || !contacts) return { usPoints: [], themPoints: [], rallyScores: [] };
    ...
    rallies.forEach((rally, idx) => {
      ...
      // Find our serve contact for this rally's time window
      const serveContact = contacts.find(
        (c) => c.action === 'serve' && !c.opponent_contact &&
               c.timestamp > prevTs && c.timestamp <= rally.timestamp
      );
      const serverJersey = serveContact ? (jerseyMap[serveContact.player_id] ?? '?') : null;
      const onOurServe   = rally.serve_side === 'us';

      if (rally.point_winner === 'us') {
        usScore++;
        usPoints.push({
          pointNum:     usScore,
          // scored on our serve → show jersey; scored on side-out → null (shows ◂)
          serverJersey: onOurServe ? serverJersey : null,
          sideOut:      !onOurServe,
        });
      } else {
        themScore++;
        themPoints.push({
          pointNum:     themScore,
          serverJersey: null,
          sideOut:      onOurServe, // they scored on our serve = side-out for us
        });
      }

      rallyScores.push({ ts: rally.timestamp, usScore, themScore });
    });

    return { usPoints, themPoints, rallyScores };
  }, [rallies, contacts, jerseyMap]);

  const processedSubs = useMemo(() => {
    if (!subs || !jerseyMap) return [];
    return subs
      ...
      .map((s) => {
        ...
        return {
          ...
          score:     prev ? `${prev.usScore}-${prev.themScore}` : '',
          ts:        s.timestamp,
        };
      })
      .sort((a, b) => a.ts - b.ts);
  }, [subs, jerseyMap, rallyScores]);

  const usScoredMap = useMemo(
    () => Object.fromEntries(usPoints.map((p) => [p.pointNum, { serverJersey: p.serverJersey, sideOut: p.sideOut }])),
    [usPoints]
  );
  const themScoredMap = useMemo(
    () => Object.fromEntries(themPoints.map((p) => [p.pointNum, { serverJersey: p.serverJersey, sideOut: p.sideOut }])),
    [themPoints]
  );
  ...
  return (
    <div className="flex-1 flex overflow-hidden min-h-0">
      <ScoringPanel
        ...
        timeouts={ourTimeouts}
        cellNums={cellNums}
        scoredMap={usScoredMap}
        currentScore={usPoints.length}
        subs={processedSubs}
        subsUsed={subsUsed}
        maxSubs={maxSubsPerSet}
        showSubs
      />
      <div className="w-px bg-slate-700 shrink-0" />
      <ScoringPanel
        teamName={opponentName || 'OPP'}
        timeouts={oppTimeouts}
        cellNums={cellNums}
        scoredMap={themScoredMap}
        currentScore={themPoints.length}
        subs={[]}
        subsUsed={0}
        maxSubs={0}
        showSubs={false}
